      ...
      for ( Army army in nation.armies ){
        ...
        if ( game.selectedarmies.contains(army) ) {
          ...
          if ( army.unitdestination != null ){
            image.beginPath();
            image.moveTo(pos.x, pos.y);
            image.lineTo(army.unitdestination.centre.x, army.unitdestination.centre.y);
            image.stroke();
          }
          if ( army.haspath ){
            ...
            for ( int i = 0; i < route.length; i++ ){
              if ( i == 0 ) { continue; }
              image.beginPath();
              image.moveTo(route[i].centre.x, route[i].centre.y );
              image.lineTo(route[i-1].centre.x, route[i-1].centre.y);
              image.stroke();
            }
            if ( army.unitdestination != null ){
              image.beginPath();
              image.moveTo(army.unitdestination.centre.x, army.unitdestination.centre.y );
              image.lineTo(route.last.centre.x, route.last.centre.y);
              image.stroke();
            }
          }
        }
